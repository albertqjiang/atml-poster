...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \begin{column}{.3\textwidth} % 1st column
    ...
    \begin{equation}
        \mathbf{m}_{uv}^t = \mathbf{A} \mathbf{h}_u^{t-1} + \mathbf{b}_A.
    \end{equation}
    ...
    \begin{equation}
        \forall v \in \mathcal{V}. \quad \mathbf{h}_v^t = \textsc{GRU}\left(\mathbf{h}_v^{t-1}, 
         + add(\{\mathbf{m}_{uv}^t \mid (u, v) \in \mathcal{E} \})\right).
    \end{equation}
    \end{block}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \begin{column}{.3\textwidth} % 2nd column
    ...
    For a fixed number of recurrent unrolling steps, \textbf{GGNN is entirely differentiable.} This was not the case for previous GNNs based on contraction maps. 
    
    Calculate gradients by back-propagation and back-propagation through time $\implies$ great boost to optimisation.
